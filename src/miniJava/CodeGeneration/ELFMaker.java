...
public class ELFMaker {
	...
	public ELFMaker(List<String> errors, long textSize, long bssSize) {
		...
		text.sh_flags = 6; // COMPLETED: what flags does the text section get? SHF_ALLOC + SHF_EXECINSTR
		text.sh_type = 1; // COMPLETED: what type is the text section?
		...
		bss.data = null;
		bss.sh_size = bssSize;
		bss.sh_type = 8; // COMPLETED: what type is the bss section?
		bss.sh_flags = 3; // COMPLETED: what are the flags of the bss section? SHF_ALLOC + SHF_WRITE
		sections.add( bss );
		
		// make .shstrtab
		shstrtab.sectionName = ".shstrtab";
		shstrtab.sh_type = 3; // COMPLETED: what is the type of the shstrtab section?
		shstrtab.sh_flags = 0; // COMPLETED: what are the flags of this section? none
	...
	public void outputELF(String fname, byte[] textSection, long entrypoint) {
		...
		phdr.p_type = 6; // COMPLETED: what is the type of the program header segment?
		phdr.p_flags = 4; // COMPLETED: what are the flags of the program header segment?
		...
		textSeg.p_type = 1; // COMPLETED: type of the text segment?
		textSeg.p_flags = 5; // COMPLETED: flags for the text segment?
	...
	private ELFSection makeNullSection() {
		...
		e.sh_name = 0;
		e.sh_type = 0; // COMPLETED: what type is the null section?
		e.sh_flags = 0;
		e.sh_addr = 0;
		e.sh_offset = 0;
		e.sh_size = 0;
